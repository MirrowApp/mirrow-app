...
| `radius` | `tuple[2] (number, number)` | `rx`<br />`ry` |  |
| `requiredExtensions` | `string` | &mdash; |  |
| `requiredFeatures` | `string` | &mdash; |  |
| `size`<br /><span class="mt-2 inline-block rounded-full bg-pink-500/20 px-2 py-0.5 text-[0.625rem] font-semibold uppercase tracking-[0.08em] text-pink-400">required</span> | `tuple[2] (number, number)` | `width`<br />`height` |  |
| `stroke` | `string` | &mdash; |  |
